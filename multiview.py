...
class multiview():
    ...
    def _total_obj_func(self):
        ...
        O_M = 1/2 * np.sum(np.multiply(self.beta,  (term_1_norm_sq + np.multiply(self.lambda_v, term_2_norm_sq)))) + self.lambda_star_f/2 * term_3_norm_sq + self.lambda_f/2 * np.sum(term_4)
    ...
    def _optimize_towards_V_star_and_W(self):
        ...
        while (iter_count < multiview._MAX_ITER)and (abs(func_val - func_val_old)/abs(func_val_old)  > multiview._TOLERANCE):
            ...
            for i in range(self.Y_train.shape[0]):  
                ...
                #term_1 = 0
                #for v in range(self.n_v):
                    #term_1         += ( -self.lambda_v[v] * self.beta[v]) * (self.V[v][i,:].dot(self.Q[v]) - V_star_old[i,:])  #W or W_old below!!!!!!!!!!
                term_1              = np.sum([-self.lambda_v[v] * self.beta[v] * (self.V[v][i,:].dot(self.Q[v]) - V_star_old[i,:]) for v in range(self.n_v)])
                term_2              = self.alpha * multiview.hinge_loss_derivative(self.Y_train[i,:].dot(self.W).dot(np.transpose(V_star_old[i,:]))) * self.Y_train[i,:].dot(self.W)
                ...
                W_der_sum          += multiview.hinge_loss_derivative(self.Y_train[i,:].dot(W_old).dot(np.transpose(self.V_star[i,:]))) * (self.Y_train[i,:]).reshape((2,1)).dot((self.V_star[i,:]).reshape((1,self.K)))
            #end_for
    ...
    def evaluate_train(self):
        ...
        def evaluate_train_sklearn(self):
            ...
            Y_predict_train_cust = np.sum(self.Y_predict_train, axis = 1)
            Y_train_cust         = np.sum(self.Y_train, axis = 1)

            confusion_matrix_   = confusion_matrix(Y_train_cust, Y_predict_train_cust)
