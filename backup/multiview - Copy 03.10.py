...
class multiview():
    ...
    regularisation_coefficients is a vector [[lambda_v], lambda_f, lambda_star_f, lambda_W]
        where:
            - [lambda_v]:    penalizes norm of the difference V*Q-V_star foe each view
    ...
    def _optimize_towards_U_and_V(self): 
        ...
        while (iter_count < multiview._MAX_ITER)and (abs(func_val - func_val_old)/func_val > multiview._TOLERANCE):
    ...
    def _optimize_towards_V_star_and_W(self):
        ...
        while (iter_count < multiview._MAX_ITER)and (abs(func_val - func_val_old)/func_val > multiview._TOLERANCE):
            ...
            for i in range(self.Y.shape[0]):  
               ...
               # print((self.Y[i,:].dot(self.W).dot(np.transpose(self.V_star[i,:]))), (self.Y[i,:]).reshape((2,1)), (self.V_star[i,:]).reshape((1,self.K)) )
